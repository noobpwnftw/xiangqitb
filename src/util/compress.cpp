...
void Compressed_Block_Store::close()
{
	...
	m_total_size.store(0, std::memory_order_relaxed);
	m_map.close_file();
	m_out.close_file();
	m_tmp_files.clear();
	m_path.clear();
	m_writer_state.store(0, std::memory_order_relaxed);
...
void Compressed_Block_Store::store(size_t block_id, Const_Span<uint8_t> data)
{
	...
	const uint64_t offset = m_total_size.fetch_add(data.size(), std::memory_order_relaxed);
