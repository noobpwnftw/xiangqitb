...
void DTM_Generator::save_egtb(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Info& info)
{
	...
	// swept in logical order instead. Flushing here hands the pages' share of
	// the budget to the compression buffers and the sweep band, keeping only
	// what the sweep can read compressed from memory.
	if (is_paged())
