	...
	public Pageable_Table
{
	...
	)
	{
		...
		m_packed.clear();
		m_packed.resize(m_pages.num_pages());
	...
	NODISCARD bool is_page_packed(size_t page) const override
	{
		return !m_packed[page].chunks.empty();
	}
	...
	NODISCARD size_t page_bytes(size_t page) const override
	{
		return page_entries(page) * ENTRY_SIZE;
	}

	NODISCARD size_t page_ram_bytes(size_t page) const override
	{
		size_t bytes = is_page_resident(page) ? page_bytes(page) : 0;
		for (const Packed_Page::Chunk& chunk : m_packed[page].chunks)
	...
	void load_page(size_t page) override
	{
		...
		if (is_page_packed(page))
		{
			...
			m_packed[page] = Packed_Page{};  // m_dirty carries over to the array
			return;
		}
	...
	void evict_page(size_t page) override
	{
		...
		if (m_dirty[page] != 0)
		{
			save_packed_page(m_packed[page], page_path(page), m_magic);
			m_dirty[page] = 0;
		}

		m_packed[page] = Packed_Page{};
	}

	NODISCARD Const_Span<uint8_t> resident_page_span(size_t page) const override
	{
		if (!is_page_resident(page))
			return Const_Span<uint8_t>();
		return Const_Span<uint8_t>(
			reinterpret_cast<const uint8_t*>(m_page_data[page].data()), page_bytes(page));
	}

	NODISCARD const Packed_Page* packed_page(size_t page) const override
	{
		return is_page_packed(page) ? &m_packed[page] : nullptr;
	}
...
private:
	...
	std::vector<Packed_Page> m_packed;
...
private:
	...
	std::vector<uint8_t> m_page_in_window[COLOR_NB];
	size_t m_max_window_pages = 1;
	...
	)
	{
		...
		const size_t max_window_loads = thread_pool->num_workers();
		...
		for (size_t page = 0; page < m_page_layout.num_pages(); ++page)
		{
			...
			{
				pins.release_all();
				for (const auto& [table, p] : m_page_pin_set)
					m_page_in_window[table][p] = 0;
				m_page_pin_set.clear();

				size_t window_loads = 0;
				for (size_t k = page; k < m_page_layout.num_pages(); ++k)
				{
					...
					const size_t slice_begin = m_page_layout.first_slice_of_page(k);
					const size_t slice_end = m_page_layout.end_slice_of_page(k);
					mark_needed_pages(me_table, slice_begin, slice_end, me_plies);
					mark_needed_pages(opp_table, slice_begin, slice_end, opp_plies);

					size_t added = 0, added_loads = 0;
					for (size_t table = 0; table < COLOR_NB; ++table)
						for (const size_t p : m_page_touched[table])
						{
							if (m_page_in_window[table][p]) continue;
							++added;
							if (!m_page_cache->is_page_resident(table, p)) ++added_loads;
						}

					if (k != page
					    && (m_page_pin_set.size() + added > m_max_window_pages
					        || window_loads + added_loads > max_window_loads))
						break;

					for (size_t table = 0; table < COLOR_NB; ++table)
						for (const size_t p : m_page_touched[table])
						{
							if (!m_page_in_window[table][p])
								m_page_pin_set.emplace_back(table, p);
							m_page_in_window[table][p] = 1;
						}
