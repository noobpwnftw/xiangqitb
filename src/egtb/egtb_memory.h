...
struct Generation_Memory
{
	...
	// Five bitsets are held while generating. WDL/DTC keeps the two unknown
	// maps while saving because finalization still consults them; DTM releases
	// the entire pool before saving its already-finalized distance tables.
	...
	// WDL/DTC additionally packs a band's WDL projection beside the raw
	// distance band. DTC finalization itself is performed in place.
	size_t sweep_output_bytes = 0;
	...
	// Compressed pages the flush may keep in memory for the sweep to read
	// instead of their spill files.
	size_t sweep_retain_bytes = 0;
	...
	NODISCARD size_t compress_peak_bytes() const
	{
		...
		return save_bitset_bytes + wdl_buffer_bytes + compression_buffer_bytes
			+ resident_distance + sweep_output_bytes
			+ sweep_retain_bytes + page_metadata_bytes;
	}
