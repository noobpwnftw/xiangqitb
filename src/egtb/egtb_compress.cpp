...
// blocks can be copied in parallel; each worker copies through local storage so
// mapping faults complete before entering the destination write path.
...
)
{
	...
	thread_pool->run_sync_task_on_all_threads([&](size_t) {
		...
		for (;;)
		{
			...
			std::memcpy(scratch.data(), block.data(), block.size());
