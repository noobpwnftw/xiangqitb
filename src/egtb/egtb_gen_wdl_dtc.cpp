...
void DTC_Generator::setup_storage()
{
	...
	printf("Paging DTC: %zu tables x (%zu slices, %zu pages of %zuMiB), %zu resident total (floor %zu)\n",
		...
		m_memory.capacity_pages, m_memory.floor_pages);
}
...
{
	Span<uint8_t> raw = sweep->reader->next_band(thread_pool);
	if (raw.empty())
	...
	for_each_band_chunk(thread_pool, entries, 1, [&](size_t, size_t first, size_t count) {
		for (size_t i = first; i < first + count; ++i)
		{
			...
			std::memcpy(raw.data() + i * DTC_File_For_Gen::ENTRY_SIZE, &f.entry, sizeof(f.entry));
		}
	});
...
void DTC_Generator::save_egtb(In_Out_Param<Thread_Pool> thread_pool)
{
	...
	if (is_paged())
	{
		...
		// files are swept in logical order instead, and the projection is
		...
		// band, keeping only what the sweep can read compressed from memory.
		page_cache().flush_all(thread_pool, m_memory.sweep_retain_bytes);
