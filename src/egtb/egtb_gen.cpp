...
)
{
	...
	for (size_t table = 0; table < COLOR_NB; ++table)
	{
		...
		m_page_in_window[table].assign(m_page_layout.num_pages(), 0);
	}
	...
	// The cache charges each page what it actually occupies, so the budget goes
	// in as bytes; a pinned page is uncompressed, which is what the floor is.
	const size_t page_bytes =
		tables.empty() ? 0 : tables.front()->page_bytes(0);
	m_max_window_pages = std::max(capacity_pages, floor);
	m_page_cache = std::make_unique<Page_Cache>(
		std::move(tables), m_max_window_pages * page_bytes);
}
